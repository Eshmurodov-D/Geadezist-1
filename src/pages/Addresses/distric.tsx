...
const District: React.FC = () => {
  ...
  const fetchDistrictsAndRegions = async () => {
    try {
      const districtResponse = await axios.get("http://142.93.106.195:9090/district", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });

      const regionResponse = await axios.get("http://142.93.106.195:9090/region", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  return (
    <div className="max-w-4xl mx-auto p-8">
      <h1 className="text-2xl font-semibold mb-6">Add New District</h1>
